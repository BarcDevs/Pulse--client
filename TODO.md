...
| 9 | `src/components/progress/share/ShareProgressCard.tsx` | — | **[Low]** Share progress in community — wire share action to community post creation. **Deferred:** forum has no image-attachment support; needs a design session to define post format (text summary vs. image) before implementation |
...
| ~~2~~ | ~~`src/config/appSettings.ts`~~                           | ~~8–11~~ | ~~**[Low]** Social links point to placeholder/wrong URLs — add proper social links. Gated behind `FEATURES.socialLinks` (off) until real account URLs are available~~ |
